...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      return toastMessage("Passwords don't match", false);
    }
    setLoading(true);
    try {
      ...
      if (data.success) {
        signup(); // Use the simplified signup to notify and redirect
      }
    } catch (error) {
      toastMessage(error.message || 'Registration failed', false);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="text-center">
            ...
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
                Join 50,000+ researchers managing their journey better.
            </p>
        </div>
      </div>
